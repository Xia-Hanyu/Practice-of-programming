...
/**
 * @brief 基本变量定义
 * @param red_seq blue_seq 双方创建武士顺序序列
*/
 ...
 * @param 
 * red_M blue_M 司令部生命元
 * red_M_temp 暂时存储武士杀死敌人后获得的生命元
 * red_cur_born 当前应制造的武士编号
 * red_num 武士序号
 * red_headquarter_enemy 进入司令部的敌军数量 
*/
...
{
    /**
    * @brief 城市信息, 城市指针管理武士, NULL代表没有武士
    * @param
    * pred 存放当前位置武士
    * temp_red 存放刚刚移动到该地点的武士
    * element 城市储存生命元
    * red_flag_cnt 连胜数量
    */
    Warrior * pred = NULL; 
    Warrior * pblue = NULL;
    Warrior * temp_red = NULL;
...
class Warrior 
{
    ...
    */
    ...
    virtual void hurted(int dam, Warrior * attacker, bool is_arrow = false) // 是否受到的是arrow伤害
    {
        strength -= dam;
        if(strength <= 0){
			strength = 0;
	...
	virtual void fight_back(Warrior * enemy) // 派生类ninja反击不同
	{
		if(dead) return;
		int dam = damage / 2;
	...
	bool evaluate_killed(Warrior * enemy, bool attacker) // 评估自己是否会被杀死
	{
...
class Dragon : public Warrior
{
    ...
    */
	...
	void yell(bool attacker) // 发动进攻且存活，士气>0.8
	{
    ...
    virtual void battle_result(Warrior * enemy) // 战斗结算
    {
        // 1 欢呼
        // (1)士气改变
...
class Lion : public Warrior
{
    ...
    */
	...
	virtual void hurted(int dam, Warrior * attacker, bool is_arrow) // lion被杀会有生命值转移
	{
...
void warrior_evaluate() // 武士评估并决定是否使用bomb
{
	for(int i = 1; i <= N; ++i)
	{
